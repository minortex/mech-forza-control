...
def cmd_switch(args):
    ...
    print(f"  XRAM[0x{ADDR_AP_CTL:04X}] AP_CTL = 0x{v:02x}  bit2={'OK' if v&4 else 'FAIL'}")
    ...
    print(f"  XRAM[0x0741] ApExist  = 0x{e57:02x}  bit0={e57&1}")
    print(f"  XRAM[0x0726] OEM9     = 0x{e30:02x}  bit7={e30>>7}")
    sw_steps = e27 & 0x7F
    if sw_steps:
        sw_status = f"{sw_steps} step(s), about {sw_steps * 2}s"
    else:
        sw_status = "EC default, about 7s observed"
    print(f"  XRAM[0x0787] SwSpeed  = 0x{e27:02x}  {sw_status}")
    print(f"  XRAM[0x07C5] Respct   = 0x{e89:02x}  bit7={e89>>7}")
    if not m["custom"]:
    ...
    print(f"  XRAM[0x0786] TCC      = {tcc_status}")
...
def cmd_status(args):
    ...
    print("[EC Status]")
    print(f"  Mode           = {label}")
    print(f"  XRAM[0x0751] CTL    = 0x{ctl:02x}")
    print(f"  XRAM[0x0726] OEM9   = 0x{oem9:02x}  bit7={oem9>>7}")
    print(f"  XRAM[0x0727] OEM10  = 0x{oem10:02x}  bit6={(oem10>>6)&1}")
    print(f"  XRAM[0x0741] AP_OEM = 0x{oem57:02x}  bit0={oem57&1} (ApExist)")
    print(f"  XRAM[0x07C6] AP_CTL = 0x{ap:02x}  bit2={(ap>>2)&1}")
...
def cmd_dump(args):
    for addr in range(1829, 1829 + 16):
        v = ec_read(addr)
        print(f"  XRAM[0x{addr:04X}] = 0x{v:02x}")
