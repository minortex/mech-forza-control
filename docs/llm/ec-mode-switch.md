...
| Enum | Value | 含义 | FAN 模式 | XRAM[1873] 值 |
|------|-------|------|----------|-------------|
...
### XRAM[1873] — 风扇控制字节 (ADDR_MAFAN_CONTROL_BYTE)
...
### XRAM[1830] bit7 — Custom 模式标志 (ADDR_AP_OEM9)

[SetFanMode](D:\UserData\Desktop\gcu\reverseCS\GCUService_decompiled\GCUService\MyControlCenter\MyFan\MyFanManager_RamFan1p5.cs) 在写完 XRAM[1873] 后写入：

`
Custom (mode==3): XRAM[1830] |= 0x80  (bit7=1)
其他模式：XRAM[1830] &= 0x7F  (bit7=0)
`

**注意：这个写入必须在 XRAM[1873] 之后，因为 EC 处理模式字节时可能会复位这个寄存器。**

### 状态解码：XRAM[1873] + XRAM[1830] bit7

`XRAM[1873] = 0x00` 本身只能说明当前是 Normal 控制字节，不能单独区分 Gaming 和 Custom。
需要结合 XRAM[1830] bit7：

| XRAM[1873] | XRAM[1830] bit7 | 解码 |
|----------|---------------|------|
| 0xA0 | 0 | Office 25W |
| 0x00 | 0 | Gaming 45W |
| 0x10 | 0 | Turbo 65W |
...
### XRAM[1831] bit6 — Customer Mode 灯 (ADDR_AP_OEM10)
...
`csharp
SetAPCustomerModeLightOn()  → XRAM[1831] |= 0x40
SetAPCustomerModeLightOff() → XRAM[1831] &= 0xBF
`

Custom 模式下 SetFanMode 调用 	his.FanTable.CustomerModeLightOn()。

### XRAM[1857] bit0 — ApExistFlag (ADDR_AP_OEM_BYTE)
...
### XRAM[1990] bit2 — RamFan1p5 AP 控制使能 (ADDR_AP_CTL)
...
- 切换模式开始时先确保 `XRAM[1990] bit2=1`，让 EC 接受 AP 侧控制。
- 写风扇表前会清掉 bit2，写完后再置回 bit2，让 EC 重载 `XRAM[3840..3935]` 风扇表。
...
- Office / Gaming / Turbo 三个固定档主要通过 XRAM[1873] 切换 25W / 45W / 65W 档位，SPL/SPPT/FPPT 由 EC/BIOS 按档位自动确定。
- 固定档下 XRAM[1923..1925] 可以读出，也可能可以写入，但实测和逆向上看意义不大；这些值不应作为固定档的配置源，也不应当作当前 SMU 限制的权威值。
- Custom 模式同样通过 25/45/65 档位选择 XRAM[1873] 的模式字节；它和前三个固定档的关键区别不是 PL 写入，而是 Custom 标志、TCC 和 AP 风扇表控制会生效。
- 如果需要真正自定义 SPL/SPPT/FPPT，使用 `ryzenadj` 直接改 SMU。`ryzenadj` 修改后 XRAM[1923..1925] 不会随之变化；这三个 EC 寄存器只能作为切换档位/模式后的 EC 侧读出值，不能当作 ryzenadj 当前限制值。

### XRAM[1989] bit7 — FanControlRespective (ADDR_FANCTL_RESP)
...
`csharp
if (bEnable) XRAM[1989] |= 0x80; else XRAM[1989] &= 0x7F;
`
...
```text
--separate    -> XRAM[1989] = 0x80
未指定       -> XRAM[1989] = 0x00
```

### XRAM[1927] — FanSwitchSpeed (ADDR_FAN_SWITCH_SPEED)
...
`csharp
for i=0..15:
    XRAM[3840+i] = fantable.CPU[i+1].UpT (最后一点=0xFF)
    if i<15: XRAM[3856+i+1] = fantable.CPU[i].DownT
    XRAM[3872+i] = fantable.CPU[i].Duty * 2  // duty 在 EC 中是百分比×2
...
- `XRAM[1830] bit7 = 1`，Custom 模式活动。
- `XRAM[1990] bit2 = 1`，AP 侧风扇管理启用。
- 写表前后通过 `XRAM[1990] bit2` toggle 触发 EC 重载。

### 风扇 RPM / Duty 读数

| 地址 | 含义 | 换算 |
|------|------|------|
| 1124/1125 | 主风扇 RPM | `XRAM[1124] * 256 + XRAM[1125]` |
| 1132/1131 | 副风扇 RPM | `XRAM[1132] * 256 + XRAM[1131]` |
...
`
1. SetFanMode(mode)
   ├── FanTable.CustomerModeLightOn()/Off()  → XRAM[1831] bit6
   ├── XRAM[1873] = mode_value
   └── XRAM[1830] bit7 = 1 (Custom) / 0 (other)

2. SetFanTable(tableName)
   ├── SetFanControlByRamFan1p5(false) → XRAM[1990] &= ~4
   ├── SetEcFanControlRespective()     → XRAM[1989] bit7
   ├── SetEcFanTable()                 → XRAM[3840..3935]
   └── SetFanControlByRamFan1p5(true)  → XRAM[1990] |= 4 (if fan table valid)

3. 固定档即使写 XRAM[1923..1925] 意义也不大，PL 主要由 EC/BIOS 按 XRAM[1873] 档位管理
4. Custom 模式的关键差异：
   ├── XRAM[1830] bit7 = 1，标记 Custom 活动
   ├── SetCpuTccOffset() → XRAM[1926]，TCC 设置生效
   └── AP 风扇表/独立风扇控制生效
5. SetFanSwitchSpeed()         → XRAM[1927]
`
...
`
1. FanTable.DisableByService() → XRAM[1990] &= ~4
2. SetFanMode(m_DefaultMode)   → XRAM[1873] = default (0 或 160)
3. SetCpuTccOffset(0, false)
4. SetFanSwitchSpeedEnabled(0) → XRAM[1927] = 0  # EC 默认渐变，实测约 7s，不是 0s
`
...
XRAM[1990] &= ~0x04                # disable
写风扇曲线到 XRAM[3840..3935]      # 6 组×16 字节
XRAM[1990] |= 0x04                 # re-enable (if fan table valid)

# 3. 自定义/非自定义标记
if mode == 3 (Custom):
    XRAM[1831] |= 0x40             # CustomerModeLightOn
else:
    XRAM[1831] &= ~0x40

# 4. Custom-only: TCC / 风扇控制
if mode == 3 (Custom):
    XRAM[1926] = 0 if tcc == 0 else (tcc | 0x80)
    XRAM[1989] = separate ? 0x80 : 0x00

# 5. FanSwitchSpeed
XRAM[1927] = 0x81                  # enable + 1 step ≈ 2s

# 6. 模式 (触发 EC 状态切换)
XRAM[1873] = mode_value            # 25W=0xA0, 45W=0x00, 65W=0x10
...
| 曲线 | EC 地址 | 传感器 | 对应风扇 |
|------|----------|--------|----------|
| CPU (XRAM[3840-3887]) | UpT/DnT/Duty | **CPU 内部 DTS (PECI)** | 1 号风扇 (XRAM[1883]) |
| GPU (XRAM[3888-3935]) | UpT/DnT/Duty | EC 固件第二路温度输入 | 2 号风扇 (XRAM[1884]) |
...
_注意：风扇独立输出（--separate）只在 XRAM[1989] bit7=1 时生效，
否则两个风扇都走 CPU 曲线。_
...
## XRAM[1926] — TCC 温度目标/使能 (ADDR_TCC_OFFSET)
...
SetUserProfile()
  └── if TccOffsetSwitch == 1:
        SetCpuTccOffset(AmdTccTarget, true)    → XRAM[1926] = AmdTccTarget | 0x80
      else:
        SetCpuTccOffset(0, false)               → XRAM[1926] = 0
```
...
对应 ryzenadj 参数：`--tctl-temp=95` 在 AMD 路径下更接近 `XRAM[1926] = 0xDF`，而不是 offset 10。

---

## XRAM[1831] bit7 — PL4 双倍标志 (ADDR_SUPPORT_CPU_PL4_DOUBLE_FLAG)

`IsSupportCPUPL4DoubleFlag()` 读取 XRAM[1831] bit7 决定 PL4 写入值是否翻倍：
...
实测本机 (Mechrevo 7735H) **XRAM[1831] bit7 = 0**，PL4 直接写入不需要 ÷2。

---

## XRAM[1875..1876] — VRM 电流限制 (ADDR_CPU_VRM_CURRENT_LIMIT)
...
## XRAM[1926..1930] — 高级 PWM 默认值（旧路径/重叠区）
...
**注意：** 这是旧 MyFan3 路径中的命名/读取逻辑。RamFan1p5 的 CPU TCC 控制也使用 XRAM[1926]，因此不要把 XRAM[1926] 当作普通 PWM 默认值写入。当前工具不依赖这组 PWM 默认值，模式切换时也不写 XRAM[1926..1930] 作为 PWM 默认表。
