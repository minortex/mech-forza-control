...
PL_DEFAULTS = {
    "gaming": (45, 45, 50), "office": (25, 25, 30),
    "turbo":  (73, 73, 90), "custom": (45, 45, 50),
}
PL_DC = (0, 0, 0)
...
MODES = {
    "office": {"desc":"Office (silent)","mode":0,"ctl":CTL_USER_HI,"pl":PL_DEFAULTS["office"],"custom":False},
    "gaming": {"desc":"Gaming (balanced)","mode":1,"ctl":CTL_NORMAL,"pl":PL_DEFAULTS["gaming"],"custom":False},
    "turbo":  {"desc":"Turbo (performance)","mode":2,"ctl":CTL_TURBO,"pl":None,"custom":False},
    "custom": {"desc":"Custom (manual fan)","mode":3,"ctl":CTL_NORMAL,"pl":PL_DEFAULTS["custom"],"custom":True},
}

_LABELS = {
    0:"Normal (Gaming/Custom)", 16:"Turbo", 64:"FanBoost",
    80:"Turbo+FanBoost", 128:"User_Fan", 160:"HiMode (Office)", 224:"HiMode+FanBoost",
}
...
def _write_fan_table(cpu=None, gpu=None):
    ...
    for i in range(16):
        ec_write(3840+i, cpu['upT'][i+1] if i<15 else 255)
        if i<15: ec_write(3856+i+1, cpu['dnT'][i])
        ec_write(3872+i, min(cpu['duty'][i],100)*2)
        ec_write(3888+i, gpu['upT'][i+1] if i<15 else 255)
        if i<15: ec_write(3904+i+1, gpu['dnT'][i])
        ec_write(3920+i, min(gpu['duty'][i],100)*2)
...
def cmd_switch(args):
    ...
    pl1, pl2, pl4 = args.pl1, args.pl2, args.pl4
    ...
    ec_rmw(ADDR_AP_CTL, clear_bits=0x04)
    ec_write(1927, 0x81)
    ec_write(1926, args.tcc | 0x80)
    ec_write(1989, 0x80 if args.separate else 0x00)
    ec_rmw(ADDR_AP_CTL, set_bits=0x04)
    ec_write(ADDR_PL1, pl1 & 0xFF); ec_write(ADDR_PL2, pl2 & 0xFF); ec_write(ADDR_PL4, pl4 & 0xFF)
    ec_write(ADDR_MAFAN_CTL, m["ctl"])
    ec_rmw(ADDR_AP_CTL, set_bits=0x04)
    ec_rmw(ADDR_AP_OEM9, set_bits=0x80) if m["custom"] else ec_rmw(ADDR_AP_OEM9, clear_bits=0x80)

    e57 = ec_read(1857); e30 = ec_read(1830)
    e27 = ec_read(1927); e89 = ec_read(1989)
    e26 = ec_read(1926); got = ec_read(ADDR_MAFAN_CTL)
